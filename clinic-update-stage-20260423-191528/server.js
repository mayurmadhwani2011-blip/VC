...
app.post('/api/admin/system-update', requireRole('admin'), async (req, res) => {
  ...
  if (!me || !isSuperAdminUser(db, me.id)) {
    return res.status(403).json({ error: 'Only Super Admin can run System Update' });
  }
  ...
  const GITHUB_REPO = 'mayurmadhwani2011-blip/CMS';
  ...
  if (useGit) {
    ...
    const tmpZip = path.join(os.tmpdir(), `clinic-update-${Date.now()}.zip`);
    const tmpExtract = path.join(os.tmpdir(), `clinic-update-${Date.now()}`);
    ...
    try {
      await new Promise((resolve, reject) => {
        const follow = (url, depth) => {
          ...
          https.get(url, { headers: { 'User-Agent': 'ClinicApp-Updater' } }, res2 => {
            ...
            if (res2.statusCode !== 200) {
              res2.resume();
              return reject(new Error(`Download failed with HTTP ${res2.statusCode}`));
            }
    ...
    // Find extracted subfolder (GitHub zips as reponame-branch/)
    ...
    const srcDir = fs.existsSync(path.join(extractedRoot, 'public', 'app.js'))
      ? extractedRoot
      : nestedClinic;
    if (!fs.existsSync(path.join(srcDir, 'public', 'app.js'))) {
      return res.status(500).json({ error: 'Update package structure is invalid (public/app.js missing)', backup, logs });
    }
    const appDir = path.resolve(__dirname);

    // Robocopy: copy new files, skip data/ and bat scripts
    const robocopyCmd = runUpdateCommand('robocopy', [
      srcDir, appDir,
      '/E', '/R:1', '/W:1',
      '/XD', path.join(appDir, '.git'), path.join(appDir, 'node_modules'), path.join(appDir, 'data'),
      '/XF', 'install.bat', 'uninstall.bat', 'upgrade-preserve-data.bat', 'upgrade-explicit-target.bat'
    ], { timeout: 120000 });
    // robocopy exit codes 0-7 are success/warning; 8+ are errors
    const rcOk = robocopyCmd.status < 8;
    pushLog('robocopy', { ...robocopyCmd, status: rcOk ? 0 : robocopyCmd.status });
    if (!rcOk) {
      return res.status(500).json({ error: `File copy failed (robocopy code ${robocopyCmd.status})`, backup, logs });
    }
    ...
    logs.push({ step: 'zip-path-complete', status: 0, stdout: 'Files updated via zip download', stderr: '' });
    afterCommit = 'zip-' + GITHUB_BRANCH;
  }

  // npm install + rebuild (both paths)
  const npmCmd = runUpdateCommand('npm', ['install', '--omit=dev'], { timeout: 300000 });
  pushLog('npm-install', npmCmd);
  if (npmCmd.status !== 0) {
    return res.status(500).json({ error: 'Dependency update failed (npm install)', backup, logs });
  }
  const rebuildCmd = runUpdateCommand('npm', ['rebuild'], { timeout: 300000 });
  pushLog('npm-rebuild', rebuildCmd);
  ...
  logActivity(db, req, {
    ...
    entity_type: 'system',
    entity_id: 1,
    notes: `System update executed from ${beforeCommit || 'unknown'} to ${afterCommit || 'unknown'}`,
    meta: {
      remote,
      branch,
  ...
  res.json({
    ok: true,
    message: 'System update completed. Restart the app service to apply new backend code.',
    remote,
    branch,
    before_commit: beforeCommit,
    after_commit: afterCommit,
...
app.put('/api/setup/printer', requireRole('admin'), (req, res) => {
  ...
  // Backward compatibility default profile printer.
